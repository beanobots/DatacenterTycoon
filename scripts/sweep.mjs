...
function evaluate(registry, scenarioId, seed) {
  ...
  for (let y = 1; y <= YEARS; y += 1) {
    ...
    if (!report) continue;
    if (report.financial.operatingProfit > 0) { if (profitable === null) profitable = y; }
    else profitable = null;
    if (debtClear === null && engine.state.company.debt < 1000) debtClear = y;
  }
...
try {
  for (const raw of values) {
    ...
    const debt = runs.filter((r) => r.debtClear !== null);
    console.log(
      ...
      + `  debtClear ${String(debt.length).padStart(2)}/${runs.length}`
      + ` @y${mean(debt.map((r) => r.debtClear)).toFixed(1).padStart(4)}`
      ...
      + `  endMargin ${(mean(runs.map((r) => r.margin)) * 100).toFixed(0).padStart(4)}%`,
    );
  }
} finally {
